...
import Countdown from "../../components/Pengaturan/Acara/Countdown";
...
const Acara = () => {
  ...
  const handleDeleteAcara = async (acaraId) => {
    try {
      const result = await Swal.fire({
        title: "Konfirmasi",
        text: "Apakah Anda yakin akan menghapus data acara?",
        icon: "warning",
  ...
  return (
    <div className=" bg-[url('/assets/hero2.png')] bg-[length:100%_270px] bg-no-repeat ">
      <h2 className="text-2xl text-primary-500 font-medium mb-4 mx-5">Daftar Acara</h2>
      <div className=" gap-5 mx-5">
        <div className="bg-white rounded-lg bg-opacity-40 shadow-lg  mb-5 w-full ">
          <Countdown fetchAcara={fetchAcara} acara={acara} />
        </div>
        <div className="w-full">
          <ul>
            ...
            <button className="flex items-center gap-1 bg-primary-500 hover:bg-primary-600 text-white p-2 rounded" onClick={toggleModalAdd}>
              <MdLibraryAdd /> Tambah Acara
            </button>
          </ul>
        </div>
      </div>
      {showModalAdd && <ModalAddAcara onClose={toggleModalAdd} fetchAcara={fetchAcara} />}
      {showModalEdit && <ModalEditAcara acara={selectedAcara} onClose={toggleModalEdit} fetchAcara={fetchAcara} />}
    </div>
  );
};
